<div class="d-flex">

  <!--Sidebar-->
  <div id="sidebar-container" class="bg-gradient">
    <div class="logo d-flex justify-content-center">
      <img src="/assets/img/medica.jpg">
      <div class="nome">
        <p class="font-weight-bold"> Admin</p>
        <p>Joana administradora</p>
      </div>
    </div>
    <div class="menu ">
      <div class="d-flex justify-content-center">
        <ion-icon name="key-outline"></ion-icon>
        <a href="#" class="d-block text-light p-3">Cadastro Admin</a>
      </div>
      <div class="d-flex pl-4 justify-content-center">
        <ion-icon name="flask-outline"></ion-icon>
        <a href="#" class="d-block text-light p-3">Cadastro Laborátorio</a>
      </div>
      <div class="d-flex justify-content-center">
        <ion-icon ion-icon name="fitness-outline"></ion-icon>
        <a href="#" class="d-block text-light p-3">Cadastro Médico</a>
      </div>
      <div class="d-flex justify-content-center">
        <ion-icon name="person-outline"></ion-icon>
        <a href="#" class="d-block text-light p-3">Cadastro Paciente</a>
      </div>
      <div class="d-flex justify-content-center">
        <ion-icon name="reader-outline"></ion-icon>
        <a href="#" class="d-block text-light p-3">Cadastro Consulta</a>
      </div>
    </div>
  </div>

  <!--TopBar-->
  <div id="topbar-container" class="shadow p-3 mb-5 bg-white rounded">
    <p class="h1">Cadastro de Médico</p>
  </div>

</div>